...
use aletheia_egregore::{
    ...
    ir::{AGENT_MEMORY_SCHEMA_VERSION, Graph, VERIFICATION_SCHEMA_VERSION, agent_memory_stable_id},
};
...
fn seed(verification_status: Option<&str>) -> Fixture {
    ...
    let file_id = agent_memory_stable_id(&["node", "File", &file_path]);
    ...
    let symbol_id = agent_memory_stable_id(&["node", "Symbol", &file_path, "foo"]);
    records.push(GraphRecord::syntax_node(
        symbol_id.clone(),
        NodeKind::Symbol,
        file_path.clone(),
    ...
    let verification_id = agent_memory_stable_id(&["node", "verification", "ver_259"]);
...
fn session_lane_returns_trust_separated_sections() {
    ...
    let touched_ids: Vec<&str> = touched
        .iter()
        .map(|r| r["record_id"].as_str().unwrap())
        .collect();
    assert!(touched_ids.contains(&fx.file_path.as_str()) == false); // record ids, not paths
    assert!(touched.iter().any(|r| r["repo_relative_path"] == fx.file_path));
    assert!(
    ...
    for section in ["touched", "observations", "failures", "artifacts", "verification_evidence"] {
...
fn ambiguous_source_handle_exits_1() {
    ...
    for (n, sess) in ["sess_a", "sess_b"].iter().enumerate() {
        ...
        {
            *schema_version = AGENT_MEMORY_SCHEMA_VERSION;
            *source_artifact_path = Some("same.traj".to_owned());
