...
impl EmbeddedAletheiaSink {
    ...
    /// Writes a byte-level `.albk` backup artifact of this store (issue #493).
    ///
    /// Thin adapter boundary over [`::aletheiadb::AletheiaDB::backup`]: captures
    /// all current nodes/edges, the full version history (including cold-tier
    /// versions), the string interner table, declared schema constraints, and
    /// the unique-constraint registry. The artifact is written atomically
    /// (temp → rename).
    ///
    /// Takes `&self`: no write lease is required. Callers back this with the
    /// throwaway-copy read-only path (`readonly_audit_store`) so a backup
    /// never opens or mutates the live store and works while a writer holds
    /// the lease.
    ///
    /// # Privacy
    ///
    /// The artifact is byte-level: it carries `eg forget` / `eg forget-repo`
    /// suppressed record bodies verbatim. It is a disaster-recovery artifact,
    /// never a sharing format — see `docs/cli/store-backup.md`.
    pub(crate) fn backup_store(
        &self,
        path: impl AsRef<Path>,
    ) -> AdapterResult<::aletheiadb::BackupSummary> {
        ...
        self.db
            ...
            .map_err(|error| AdapterError::Rejected {
                record_id: "embedded-store".to_owned(),
                message: format!(
                    "failed to write backup artifact {}: {error}",
                    path.display()
                ),
            })
    }

    /// Restores a `.albk` backup artifact into a data directory (issue #493).
    ///
    /// Thin adapter boundary over
    /// [`::aletheiadb::AletheiaDB::restore_to_data_dir`], which fail-closes on
    /// a non-empty target (`TargetNotEmpty`): restore only ever populates a
    /// missing or empty directory, never merges into or overwrites a live
    /// store. The returned database is dropped after the durable layout is
    /// materialised; reopen through [`Self::open`] to use it.
    ///
    /// # Errors
    ///
    /// Returns [`AdapterError::Rejected`] when the artifact is missing, not a
    /// valid backup, or newer than the reader window, and when the target
    ...
    pub(crate) fn restore_store(
        ...
        let data_dir = data_dir.as_ref();
        ::aletheiadb::AletheiaDB::restore_to_data_dir(from, data_dir)
            .map(|_db| ())
            .map_err(|error| AdapterError::Rejected {
                record_id: "embedded-store".to_owned(),
                message: format!(
